...
MAX_PREDICTIONS_24H   = 0     # 0 = بلا سقف (أمر المالك 2026-08-29 بعد حالة
                              # غلطاسراي: سبت مزدحم تجاوز سقف الـ150 القديم
                              # فقُصّت مباريات المساء غير الصدارة منهجياً —
                              # «كل ما هو موجود يُحمى». الكلفة الثقيلة لها
                              # سقوفها الخاصة الباقية (الإثراء والنداءات)؛
                              # هذا يفتح التوقع الخفيف وحده. يبقى متطابقاً
                              # في المحركين — أساس عدالة المقارنة
    ...
    for fid in list(pending.keys()):
        ...
        if status in FINAL_STATUSES and gh is not None and ga is not None:
            ...
            entry = {
                ...
                # 🥇 وسم التحقق السوقي (HOLD-016): يُحفظ مع النتيجة وإلا استحال
                # قياس «المتوقع مقابل المقاس» بعد 30 ذهبية — كان يسقط هنا
                # (اكتُشف 2026-09-12: صفر صفوف موسومة في resolved بعد 3 أيام)
                "gold_check": p.get("gold_check"),
...
def get_upcoming_24h() -> list:
    ...
    # 0 = بلا سقف: كل المباريات الصالحة تُتوقَّع (أمر المالك 2026-08-29)
...
def main() -> None:
    ...
    # (فجوة صبيحة 2026-08-17). **الخروج مؤجَّل إلى آخر التشغيلة** (إصلاح
    # 2026-09-13): الخروج هنا كان يسبق الأرشيف الدائم وحارس النزاهة، فصباح
    # الرفض الشامل لم يتقدم history.json وأطلق الحارس الخارجي «لم يجرِ
    # التقييم الصباحي» رغم أن التقييم جرى كاملاً — الرفض يُحمّر التشغيلة
    # بعد أن يُكتب كل ما نجح، لا قبله.
    total_refusal = ""
    if upcoming and not new_preds:
        total_refusal = f"⛔ {len(upcoming)} مرشحاً وصفر توقعات محفوظة — رفض Claude شامل"
        print(total_refusal)
    ...
    if total_refusal:
        raise SystemExit(total_refusal)
