...
- **Lessons loop (Phase 3 — LIVE)**: every morning, after grading, `generate_lessons()` sends up to `MAX_MISTAKES_PER_RUN=10` of yesterday's WRONG predictions to Claude, which extracts one short, generalizable Arabic lesson per mistake (a pattern to watch for, not a description of the game). Lessons are stored in `lessons_v2.json` as `{"date","match","text"}` (capped at `MAX_LESSONS_STORED=100`); the most recent `MAX_LESSONS_IN_PROMPT=15` are injected into every prediction prompt under "دروس من أخطائك السابقة". The newest 10 also appear on the dashboard (V2 tab) and the digest notes how many new lessons were learned.
...
4. **API budget awareness:** monitor ≈72 calls/day for polling + up to 18/run for Engine 2 live context (3 calls × ≤6 top-league matches, only when there's a start/goal event — realistically a few hundred/day worst case), predict ≤5/day (2 fixture-date calls + up to 3 resolve calls), predict_v2 ≤130/day (same 5 + enrichment capped at 120). Pro plan allows 7,500/day so there is huge headroom now, but keep calls efficient and batched.
