...
- **Engine 2 live pulse (نبض المحرك 2)**: for watchlist matches ONLY, every monitor cycle with no goal/start/FT event runs a pulse — `live_pulse()` in `monitor.py` fetches full live details (3 API calls) and asks `claude-fable-5` (extended thinking) whether a NEW significant scenario is forming vs its previous reading (next goal & side, both-teams-to-score, corner/set-piece wave, named dangerous player, likely card, momentum flip, leader closing the game). Reply "لا جديد" → silence; otherwise a "👁 عين المحرك 2" alert is sent. The last reading is stored in `state.json` (`pulse` field; a goal's enriched analysis becomes the new baseline) so each pulse compares against the previous one — no repeated alerts for the same situation. Caps: `MAX_PULSE_PER_RUN=6` Claude calls/run, statuses `1H/2H/ET` only (no HT/penalties).
