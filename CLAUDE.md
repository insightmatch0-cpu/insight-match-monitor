...
- **GitHub's cron is best-effort, not guaranteed**: on 2026-07-14 GitHub silently skipped both daily prediction runs (03:15/03:30 UTC) and left an 8.5h gap in the 20-min monitor cron. Permanent mitigation (four layers, keep all of them): (1) the scheduler watchdog in every monitor run, (2) backup cron slots one hour after the primary with a same-day guard, (3) duplicate runs are harmless by design — resolve is idempotent, already-pending fixtures are skipped, and no digest is sent when there is nothing new, (4) **external heartbeat**: three permanent Claude Routines (staggered :05/:25/:45 hourly, named "InsightMatch pinger A/B/C") live OUTSIDE this repo in the owner's Claude session infrastructure — each checks the latest monitor.yml run and re-triggers it if older than 15 min, and fires missed daily prediction runs in V1-then-V2 order. If those Routines are ever lost, the even stronger alternative is an external cron service (cron-job.org) POSTing to the workflow-dispatch API with a fine-grained PAT (Actions:write, this repo only) — the owner knows the setup steps.
